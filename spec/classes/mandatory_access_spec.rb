...
describe 'toughen::mandatory_access' do
    ...
    context 'with mode set to permissive' do
        ...
        it {
            should contain_augeas('selinux-mode').with({:changes => 'set SELINUX permissive'})
        }
    end
    ...
    context 'with policy set to multiuser' do
        ...
        it {
            should contain_augeas('selinux-policy').with({:changes => 'set SELINUXTYPE multiuser'})
        }
    end

    context 'with setroubleshoot installed' do
        let (:params) do
            { :setroubleshoot_ensure => 'installed' }
        end
        it {
            should contain_package('setroubleshoot').with({:ensure => 'installed'})
        }
    end

    context 'with mcstrans installed' do
        let (:params) do
            { :mcstrans_ensure => 'installed' }
        end
        it {
            should contain_package('mcstrans').with({:ensure => 'installed'})
        }
    end
end
